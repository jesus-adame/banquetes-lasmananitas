...
function mostrarLogistica(data) {
	...
	for (let val of data) {
		...
		fecha = new Date(fechahora[0]),
		date_format = (fecha.getDate() + 1) + '-' + meses[fecha.getMonth()] + '-' + fecha.getFullYear();
...
function mostrarOrdenes(data) {
	...
	for (let val of data) {
		...
		<td><a href="orden_pdf&id=${val.id_orden}" target="_blank">${val.id_orden}</a></td>
		<td>${date_format}</td>
		<td>${val.orden}</td>
		<td>${val.lugar}</td>
		<td>
